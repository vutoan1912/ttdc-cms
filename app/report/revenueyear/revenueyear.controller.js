angular
    .module('erpApp')
    .controller('RevenueYearController', RevenueYearController);

    RevenueYearController.$inject = ['$scope', '$rootScope', '$state', '$timeout', '$compile',
    ...
    function RevenueYearController(
        ...
        $scope.myColumnsRd = ["Gói dịch vụ", "Doanh thu", "Doanh thu năm trước", "Tỷ lệ", "Thời gian"];
        var fieldsRd =       ["cmd_code", "dt",       "dt_namtruoc", "tyle",   "action_time"];
        var fieldsTypeRd =   ["Text",     "Number",   "Number",      "Number", "DateTime" ];
        $scope.fieldsValue =  [null,       null,       null,          null,     null      ];
        var loadFunctionRd = ReportService.getRevenueYear;
        ...
        $scope.getData = function () {
            ...
            var req = {
                method: 'GET',
                url: API_URL + '/api/sms/getDoanhThuNam?' + query,
                headers: {
