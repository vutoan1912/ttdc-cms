angular
    .module('erpApp')
    .controller('RevenueWeekController', RevenueWeekController);

    RevenueWeekController.$inject = ['$scope', '$rootScope', '$state', '$timeout', '$compile',
    ...
    function RevenueWeekController(
        ...
        $scope.myColumnsRd = ["Gói dịch vụ", "Doanh thu tuần", "Doanh thu tuần trước", "Lũy kế tháng", "Tỷ lệ", "Tỷ lệ lũy kế", "Thời gian"];
        var fieldsRd =       ["cmd_code", "dt_tuan",  "dt_tuantuoc", "luyke_thang",  "tyle",  "tyle_luyke", "action_time"];
        var fieldsTypeRd =   ["Text",     "Number",   "Number",      "Number",       "Number","Number",     "DateTime"];
        $scope.fieldsValue =  [null,       null,       null,          null,           null,    null,         null];
        var loadFunctionRd = ReportService.getRevenueWeek;
        ...
        $scope.getData = function () {
            ...
            var req = {
                method: 'GET',
                url: API_URL + '/api/sms/getDoanhthuTuan?' + query,
                headers: {
        ...
        $scope.chooseDatetime = function () {
            if($scope.DateTimeRange.kstartDateTime != null){
                ...
                $scope.TABLES[newTableIdsRd.idTable].param_filter_list[6] = 'date='+startResult;
            }else{
                $scope.TABLES[newTableIdsRd.idTable].param_filter_list[6] = '';
            }
        };
