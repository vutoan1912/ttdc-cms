angular
    ...
    .controller('LotHomeController', [
        ...
        'apiData',
        '$http',
        function ($scope, $rootScope, $timeout, $compile, Lot, $stateParams, $interval, TableMultiple, $translate, $window, AlertService, $state, $filter, Sequence, Location, Warehouse, ErrorHandle, apiData, $http) {
            ...
            $scope.CbxActivate = {
                activateService: Lot.activate,
                deactivateService: Lot.deactivate
            }
            ...
            var loadFunction = Lot.getPage;
            var fields = ["id", "lotNumber","lotType","location","company", "created", "createdBy", "updated", "updatedBy", "active"];
            ...
            $scope.activate = function () {
                if ($scope.TABLES[newTableIds.idTable].param_check_list.length > 0) {
                    Lot.activate($scope.TABLES[newTableIds.idTable].param_check_list).then(function () {
            ...
            $scope.deactivate = function () {
                if ($scope.TABLES[newTableIds.idTable].param_check_list.length > 0) {
                    Lot.deactivate($scope.TABLES[newTableIds.idTable].param_check_list).then(function () {
            ...
            $scope.deleteList = function (table_id) {
                ...
                UIkit.modal.confirm($translate.instant("common-ui-element.actionConfirm.Delete"), function () {
                    Lot.deleteRecord($scope.TABLES[table_id].param_check_list).then(function (data) {
                        if (data.length > 0) {
            ...
            $scope.myColumns = ["Lot/Serial Number", "Lot Type", "Location","Companies","Created", "Created By", "Update", "Updated By", "Active"]
            $scope.myColumnsShow = []
